...
#include "CEQS_ContextStrafeRight.h"
...
void UCEQS_ContextStrafeRight::ProvideContext(FEnvQueryInstance& QueryInstance, FEnvQueryContextData& ContextData) const
{
	...
	if (QueryInstance.Owner.IsValid())
	{
		...
		else
		{
			...
			else
			{
				...
				FRotator lookAtRotation = FRotator(0, 0, 0);
				// 1. 타겟 바라보는 방향에서 이동 방향 얻기 (우측 + 90)
				lookAtRotation.Yaw = UKismetMathLibrary::FindLookAtRotation(ownerLocation, targetLocation).Yaw + 90.0f;
				//CLog::Print(lookAtRotation.Yaw);
