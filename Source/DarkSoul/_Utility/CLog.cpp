...
void CLog::Print(bool val, float time, FColor color, int32 key)
{
	if (val)
	{
		GEngine->AddOnScreenDebugMessage(key, time, color, FString::Printf(L"TRUE"));
	}
	else
	{
		GEngine->AddOnScreenDebugMessage(key, time, color, FString::Printf(L"FALSE"));
	}
}
...
void CLog::Print(FQuat val, float time, FColor color, int32 key)
{
	GEngine->AddOnScreenDebugMessage(key, time, color, FString::Printf(L"X : %f, Y : %f, Z : %f, w : %f", val.X, val.Y, val.Z, val.W));
}
